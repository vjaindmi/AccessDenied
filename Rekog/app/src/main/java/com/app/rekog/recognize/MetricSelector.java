...
public class MetricSelector extends FrameLayout {
    ...
    void initContent(LayoutInflater inflater, Resources res, String packageName) {
        ...
        if (isEmoji) {
            ...
            videoResourceURIs[0] = Uri
                    .parse(String.format("android.resource://%s/%d", packageName, videoId));
            videoResourceURIs[1] = Uri.parse(String
                    .format("android.resource://%s/%d", packageName, res.getIdentifier(resourceName + "0", "raw", packageName)));
        } else {
